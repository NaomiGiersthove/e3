<?php 
    ...
    $hapjes = $db->select(" SELECT menuitems.ID, menuitems.Code, menuitems.Naam, menuitems.Prijs ,gerechtsoorten.ID AS soort_ID ,gerechtcategorieen.Naam AS soort, gerechtsoorten.Naam AS soort_naam FROM menuitems 
    ...
    WHERE Gerechtcategorie_ID = 2",[]);

    $columns = array_keys($hapjes[0]);
    $row_data = array_values($hapjes);

    $soorten = $db->select("SELECT * FROM gerechtsoorten WHERE Gerechtcategorie_ID = 2",[]);
...
<body>
	...
	<table>
		...
		<tbody><h1>Eten aanpassen of verwijderen</h1>
			<?php foreach ($row_data AS $data) { ?>
				<form method="post">					
					<tr>
						<input type="hidden" name="item_id" value="<?php echo $data["ID"] ?>">

						<td><input type="text" maxlength="4" name="code" value="<?php echo $data["Code"] ?>"></td>
