...
class database{
    ...
    public function add_reservering($naam, $telefoon, $email, $tafel, $datum, $tijd, $aantal, $allergieen, $opmerkingen) {
        try{
            ...
            if (is_array($result) && count($result) > 0) { 
                echo '<script>alert("No-show!")</script>';
            }

            $sql = "INSERT INTO klanten VALUES (NULL, :naam, :telefoon, :email";
    ...
    public function edit_reserveringen($reservering_id, $klant_id, $naam, $telefoon, $email, $tafel, $datum, $tijd, $aantal, $status, $datum_toegevoegd, $allergieen, $opmerkingen) {
        try{
            // Zoekt data op uit de database
            $sql = "UPDATE reserveringen INNER JOIN klanten ON reserveringen.Klant_ID = klanten.ID SET reserveringen.Tafel = :tafel, reserveringen.Datum = :datum, reserveringen.Tijd = :tijd, reserveringen.Aantal = :aantal, reserveringen.Status = :status, reserveringen.Datum_toegevoegd = :datum_toegevoegd, reserveringen.Allergieen = :allergieen, reserveringen.opmerkingen = :opmerkingen, klanten.Naam = :naam, klanten.Telefoon = :telefoon, klanten.Email = :email WHERE reserveringen.ID = :reservering_id";
    ...
    public function edit_klanten($klant_id, $naam, $telefoon, $email) {
        try{
            // Zoekt data op uit de database
